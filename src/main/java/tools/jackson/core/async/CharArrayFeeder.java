 ...
 * as {@code char} arrays.
 */
public interface CharArrayFeeder extends NonBlockingInputFeeder
{
     /**
      * Method that can be called to feed more data, if (and only if)
      * {@link #needMoreInput} returns true.
      *
      * @param data Char array that contains data to feed: caller must ensure data remains
      ...
      * @param end Offset after last char contained in the input array
      *
      ...
      */
     public void feedInput(char[] data, int offset, int end) throws JacksonException;
}
